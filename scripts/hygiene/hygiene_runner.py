...
"""Engineering hygiene runner — single CLI index for skill-orchestration-os.

Thin delegating index (same contract as msb-v3/SOE/domain-router): every
experiment lives in its own standalone runner; this file runs them and
aggregates the verdicts into a factory-style weakest-verdict gate. The hermes
factory invokes this with `--all --json`.

Experiments (serverless — this is a CLI runtime, no daemon to probe):
  o01_orchestration_smoke  scripts/smoke_test.py (15 tests, zero-spend) — the
                           OS's own regression suite as a hygiene leg, so the
                           hygiene index and the factory's pytest_target share
                           one source of truth.

Usage:
    python hygiene_runner.py --all                 # run every experiment
    python hygiene_runner.py o01                   # run one experiment
    python hygiene_runner.py --only o01            # run a subset
...
EXPERIMENTS: dict[str, Path] = {
    "o01_orchestration_smoke": HERE / "o01_orchestration_smoke_runner.py",
}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="skill-orchestration-os hygiene runner (delegating index)")
